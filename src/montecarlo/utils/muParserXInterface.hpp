...
public:
  ...
  /*!
     * Factory method to create an interface from a text file.
     * Reads the first line of the file as the expression.
     *
     * @param filename Path to the file containing the expression
     * @return A constructed muParserXInterface object
     */
  static muParserXInterface fromFile(const std::string &filename)
  {
    ...
    if (!file.is_open())
    {
      throw std::runtime_error("Cannot open function file: " + filename);
    }

    std::string expression;
    // Legge l'intero contenuto o solo la prima riga.
    // Qui leggiamo la prima riga non vuota.
    ...
    if(expression.empty()) {
      throw std::runtime_error("File is empty or contains no expression: " + filename);
    }

    std::cout << "Read expression from file: " << expression << std::endl;

    // Chiama il costruttore normale passando la stringa letta
    return muParserXInterface(expression);
  }
